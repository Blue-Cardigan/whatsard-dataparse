...
async function processDebateForDate(dateString, type = 'commons', useSupabase = false) {
  ...
  for (const debateType of types) {
    try {
      ...
      const result = await processDebateType(dateString, debateType, xmlData, useSupabase);
...
async function processDebateType(dateString, type, xmlData, useSupabase = false) {
  ...
  // Only store in Supabase if explicitly requested
  if (useSupabase) {
    ...
    console.log(`Successfully processed ${type} debate for ${dateString} (skipping Supabase storage)`);
  }
...
async function processDebatesForPeriod(startDate, numberOfDays = 1, type = 'commons', useSupabase = false) {
  const types = Array.isArray(type) ? type : [type];
  const results = {};
  ...
  for (let day = 0; day < numberOfDays; day++) {
    ...
    try {
      const dateResults = await processDebateForDate(dateString, types, useSupabase);
      if (dateResults) {
...
if (require.main === module) {
  ...
  if (types.length === 0) types = ['commons'];
  
  processDebatesForPeriod(date, numberOfDays, types, useSupabase)
    .then(results => {
      if (results) {
