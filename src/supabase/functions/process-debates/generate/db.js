  ...
  for (const result of results) {
    ...
    if (response.status_code === 200) {
      ...
      if (type === 'speeches') {
        ...
        // Fetch the original speeches to compare lengths and speaker names
        ...
        const originalSpeeches = originalDebate.speeches;
        const rewrittenSpeeches = content.speeches;

        const speechesParallel = 
          Array.isArray(originalSpeeches) && 
          Array.isArray(rewrittenSpeeches) &&
          originalSpeeches.length === rewrittenSpeeches.length &&
          originalSpeeches.length > 0 &&
          originalSpeeches.every((speech, index) => 
            speech.speakername === rewrittenSpeeches[index].speakername
          );

        updateData.speechesparallel = speechesParallel;

        console.log(`Set speechesparallel to ${speechesParallel} for debate ID ${id} in ${debateType}`);
